...
func main() {
	...
	if *fphTimeoutMs > 0 {
		// Drain timeout alone is a no-op without the install flag — FC returns
		// 400 from start_balloon_hinting if FreePageHinting wasn't configured.
...
func run(ctx context.Context, buildID string, iterations int, coldStart, noPrefetch, noEgress, verbose, shell bool, pauseOpts pauseOptions, runOpts runOptions, fphBenchOpts fphBenchOptions) error {
	...
	sbxCfg := sandbox.NewConfig(sandbox.Config{
		...
		Vcpu:           1,
		RamMB:          512,
		// Bench compares FPR-only vs FPR+FPH; both modes need FPR armed at install.
